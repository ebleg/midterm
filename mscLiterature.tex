\documentclass{mscLiterature}
%
% Thesis data
%\mscDepartment{Delft Center for Systems and Control (\textsmaller{DCSC})}%
\mscProgram{Systems and Control}
%Change if needed to
%\mscProgram{Mechanical Engineering}
\mscFaculty{Mechanical, Maritime and Materials Engineering (3mE)}%
\mscName{E. B. Legrand}%
\mscDate{\today}%
\mscTitle{A Survey of Hyperbolic Rotations}%
\mscSubTitle{Towards a Consistent Interpretation of Financial Instruments in Economic Engineering}%
\mscKeyWords{thesis, msc, subject}% only used in PDF properties
%\mscCoverPicture{STYLESTUFF/escher-2}% to place a picture ( here the example COVER.eps) on the back of the cover page
%
%
% Third party options (create text/logo on the copywrite page)
\mscThirdPartyText{The work in this literature study was supported by Rabobank. Their cooperation is hereby gratefully acknowledged.}
\mscThirdPartyLogo{STYLESTUFF/RABO_LOGO}
% NOTE: on the title page only the TU Delft logo is permitted.
%
%
%
% Finalize the thesis data
\setThesisInfo
%
% Use \includeonly{} to build only certain parts of your thesis
%\includeonly{introduction, real_chapter, empty_chapter, long_chapter}%
%
%PH Toegevoegd 24-10-2011
%allow (matlab) listing max 1pt flexibility between lines
\lstset{lineskip=0pt plus 1pt minus 0pt}

%% -------------------------------------------------------------------

\input{preamble}

% TikZ externalization - must be in main file!
\tikzexternalize[prefix=media/cache/]

\begin{document}
%
%========================== Front matter ======================================
\frontmatter %
%
% Make the cover page and hell of a lot of title pages
\maketitle
%
\input{formalities/abstract}
%
% table of contents, (\toc of \toclof of \tocloflot )
\tocloflot
%
%
%\include{formalities/preface}
%\input{formalities/acknowledgements}
%\include{formalities/dedication}
%
%========================== Main matter ======================================
\mainmatter

\input{main/introduction}
\part{Hyperbolic Rotations in Mathematics and Physics}
\label{part:one}
\input{main/relativity}
\input{main/hyperbolic_geometry}
\input{main/mobius_transforms}
\part{Hyperbolic Rotations in Economic Engineering}
\label{part:two}
\input{main/ee}
\input{main/finance_rotation}
\input{main/research_proposal}
\input{main/summary_conclusion}

%========================== Appendices =======================================
\appendix
%

%========================== Back matter ======================================

\backmatter
%
% Bibliography
\printbib{library}
%
\input{formalities/glossary}
% test
% Index
\cleardoublepage
\printindex
%%%%%

\end{document}

...
where \lsymb{\ecokin}{Kinetic co-energy} is the kinetic co-energy\footnote{As will become clear later, the term `co-energy' is used to distinguish between the definition of kinetic energy in terms of the generalized momenta, which is the perspective of Hamiltonian mechanics.} of the system and \lsymb{\epot}{Potential energy} the potential energy. If \(M\) is a Riemannian manifold and its Lagrangian has the aforementioned form, the system is called \emph{natural} \cite{Arnold1989}.
...
\paragraph{Action-angle coordinates} A special choice of canonical coordinates arises for conservative systems, called the \emph{action-angle coordinates}. These are pairs of coordinates of the form $(\vec{I}, \vec{\phi})$, where $\vec{I}$ consists of the action coordinates, and $\vec{\phi}$ are the angle coordinates. The reason why this set of coordinates exists in the first place is found in Liouville's theorem, which states that when a system with $n$ degrees of freedom has $n$ independent first integrals \lsymb{$\vec{F}$}{First integrals of a Hamiltonian system}, then the level set of these first integrals is diffeomorphic to the the $n$-dimensional torus \lsymb{$\mathbb{T}^n$}{torus of dimension $n$}. This torus is completely parameterized by the $n$ action variables, while the angle coordinates determine the position on the torus. \Cref{fig:action_angle} visualizes the action-angle coordinates for the simple case of a system with one degree of freedom.
...
\begin{thmblock}{Noether's theorem}
    If the Lagrangian system $(M, \lag)$ admits the one-parameter group of transformations $h^s: M \to M, s \in \real$, then the Lagrangian system of equations corresponding to $\lag$ has a first integral: $I: TM \to \real$. In local coordinates $q$ on $M$, the integral $F$ is written in the form \cite{Arnold1989}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
with $t' = \dv{t}{\tau}$ and $\gpos' = \dv{\gpos}{\tau}$. This is the negative of the Legendre transformation of the Lagrangian, or the negative of the Hamiltonian; $I = -H$. Therefore, the total energy of the system is conserved. 

...
\begin{thmblock}{Poisson brackets}
    The \emph{Poisson bracket} $\poisson{E}{F}$ of functions $E$ and $F$ given on a symplectic manifold $(M^{2n}, \omega^2)$ is the derivative of the function $F$ in the direction of the phase flow with hamiltonian flow $H$:
        $$ \poisson{E}{F}(x) = \left. \dv{}{t}\right|_{t = 0} F(g^t_H(x)) $$ 
    where $g$ is the one-parameter group of canonical transformations of the manifold $M^{2n}$. The Poisson bracket is skew-symmetric and bilinear.
\end{thmblock}

...
Analytical mechanics, more specifically \emph{Lagrangian} and \emph{Hamiltonian} mechanics, are established around the definition of special state functions, respectively called the Lagrangian \lsymb{\(\lag\)}{Lagrangian} and the Hamiltonian \lsymb{\(\ham\)}{Hamiltonian}. As per usual, Lagrangian mechanics is introduced first in \cref{sec:lagrangian}, for it has the most intuitive explanation. Then, in \cref{sec:hamiltonian}, a more formal approach allows to (Legendre) transform the discussion into one of Hamiltonian mechanics. 
...
In this chapter, frequent analogies are made between mechanics and economics; as such, the idea is to have the `normal text' pertain mostly to the discussion of classical mechanics and to provide the analogies in special `boxes' like so:
\begin{econ}{Example}
...
Central to the concept of Lagrangian mechanics is the so-called \emph{configuration space} \lsymb{\(M\)}{Configuration manifold}, which is an \(n\)-dimensional manifold provided with some parameterization called \emph{generalized coordinates} assembled in the coordinate tuple \(\gpos\). 
...
\begin{figure}[ht]
    ...
    \caption{Schematic of a two-dimensional (configuration) manifold $M$ embedded in $\real^3$; the local generalized velocities associated with a point $\vec{x}$ are vectors that live in the tangent space \others{$TM_{\vec{x}}$}{Tangent space to the manifold $M$ at the point $\vec{x}$}.}
    \label{fig:conf_mnfold}
\end{figure}

\begin{econ}{The configuration manifold and the economic system}
    In economic engineering, \lsymb{\(\gpos\)}{Generalized position / stock levels} has a similar intuitive notion, namely the stock values of various products, which denote the `position' of some economic system. The name `position' may be misleading when intuitively ported to the familiar three-dimensional space; the configuration space usually has less structure, such as the absence of a metric or an inner product.\\~\\
...
\begin{thmblock}{Hamilton's principle}
    Motions $\gamma: \real \to M$ of a mechanical system coincide with extremals of the action functional \gsymb{$\Phi$}{Action functional}
    \begin{equation}
...
\end{thmblock}
The Lagrangian \(\lag\) is a mapping from the \emph{tangent bundle} \others{\(TM\)}{Tangent bundle of the manifold $M$} of the configuration manifold \(M\), optionally paired with a time argument for time-varying problems to the reals,
    \[ \lag : TM \times \real^+ \to \real, \]
i.e. it takes a generalized position \(\gpos\) and a generalized velocity \lsymb{\(\gvel\)}{Generalized velocity / flow of goods} (which live in the tangent space of \(M\)), and a time instance to some scalar values.
...
The aforementioned principle is for now only helpful on a conceptual level because it does not show how to arrive at any solutions. For this, a branch of mathematics called the calculus of variations comes to aid, which is concerned with finding extremals of \emph{functionals}\footnote{A functional is a real-valued function on a vector space (of functions).}, in this case, \(\Phi\). A necessary condition for \(\Phi\) to attain an extremal is that 
\[ \var{\Phi} = 0,\]
...
This yields a total of \(n\) second-order differential equations, or equivalently, a system of \(2n\) first-order equations. A special significance is assigned to the (co)vector $\pdv{\lag}{\gvel}$, defined as the \emph{generalized momentum} \lsymb{$\gmom$}{Generalized momentum / price levels}.
...
where \lsymb{\ecokin}{Kinetic co-energy} is the kinetic co-energy\footnote{The term `co-energy' is used to distinguish between the definition of kinetic energy in terms of the generalized momenta, which is the perspective of Hamiltonian mechanics.} of the system and \lsymb{\epot}{Potential energy} the potential energy. If \(M\) is a Riemannian manifold and its Lagrangian has the aforementioned form, the system is called \emph{natural} \cite{Arnold1989}.
...
\begin{econ}{Potential energy and convenience yield}
...
In mechanics, the Hamiltonian corresponding to $\lag = \ecokin - \epot$, where $\ecokin$ is the quadratic form given in \cref{eq:kinetic_energy}, the Hamiltonian is equal to:
\begin{equation}
    \ham = \ekin + \epot,
\end{equation}
...
\index{Hamilton's equations}
\index{cotangent bundle}
\index{Hamiltonian!time invariance}

Whereas Lagrangian mechanics acts on the tangent bundle of the configuration manifold, Hamiltonian mechanics takes place in the \emph{cotangent bundle} $T^*M$, for it is expressed in terms of the momenta rather than the generalized velocities. The generalized momenta live in the cotangent space at a certain point of the configuration manifold because they are the differential of a scalar function (the Lagrangian) on the manifold. The cotangent bundle is a vector bundle and thus a manifold in its own right; here the base manifold of the cotangent bundle is the configuration manifold $M$, the fibers are the cotangent space for each point. A natural 1-form can be defined on this cotangent bundle that exploits the distinction between the base manifold and the fibers; namely a 1-form on the co-tangent bundle that is `equivalent' to the covector in the cotangent space (with respect to their action on the tangent vector to the cotangent bundle and configuration manifold respectively). The action of this natural 1-form, referred to as the \emph{tautological 1-form}, on a tangent vector to the cotangent bundle is equal to the action of the covector on this tangent vector \emph{under the pushforward of the projection map}. That is, if \gsymb{$\pi$}{Projection map of a fiber bundle}$:\,T^*M \to M$ is the projection map, $\vec{\xi} \in T(T^*M)$ is a tangent vector to the cotangent bundle, and $m$ is an element of the cotangent bundle, the tautological 1-form \gsymb{$\omega^1$}{tautological 1-form} is defined by \cite{Arnold1989,Abraham1978}
...
\index{cotangent bundle}
\index{tautological 1-form}
\index{Poincaré!1-form|see {tautological 1-form}}
\index{canonical 1-form|see {tautological 1-form}}
\index{fiber bundle}

The exterior derivative of the Poincaré 1-form is a closed 2-form that is called the \emph{symplectic form} \gsymb{$\omega^2$}{symplectic 2-form}:
$$ \omega^2 = \dd{\omega^1}.$$
...
\index{symplectic 2-form}
\index{Hamiltonian vector field}
...
\begin{econ}{Action-angle coordinates and the business cycle}
    ...
    \begin{center}
        \input{media/tikz/business_cycle}
    \end{center}
